...
df = pd.read_csv('C:\Users\Joss\ownCloud\Shared_Read_Only\DP_Anonymized_Data\P100 - 2017-09\dpDatP100midterm.csv')
...
for index, row in df.iterrows():
    ...
    for iq in qnums:
        question = 'A'+str(iq) 
        trtcol = 'A'+str(iq)+'.trt'
        qnumstring = 'P100.17W1.midterm.A'+str(iq)
        qnumstringshort = 'm0171.'+str(iq)
        course = 'P100'
        term = '2017W1'
        exam = 'midterm'
...
colnames = ['ID','QNUM.0','QNUM.long','QNUM','COURSE','TERM','EXAM', \
    'QCORRECT','TREATMENT','SoloTotal36.M0', \
    'Version','Gender','EYAinclude','d.version','NCRT']

# now make a dataframe out of it
df2 = pd.DataFrame(array2,columns=colnames)

df2.to_csv('C:\Users\Joss\ownCloud\Shared\DP_Derived_Data\dpLogisticDat_P100MT.csv',index=False,header=True)
