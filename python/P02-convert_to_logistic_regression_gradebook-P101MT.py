...
df = pd.read_csv('C:\Users\Joss\ownCloud\Shared_Read_Only\DP_Anonymized_Data\P101 - 2018-01\dpDatP101midterm.csv')
...
for index, row in df.iterrows():
    ...
    for iq in qnums:
        question = 'A'+str(iq) 
        trtcol = 'A'+str(iq)+'.trt'
        qnumstring = 'P101.17W2.midterm.A'+str(iq)
        qnumstringshort = 'm1172.'+str(iq)
        course = 'P101'
        term = '2017W2'
        exam = 'midterm'
...
colnames = ['ID','QNUM.0','QNUM.long','QNUM','COURSE','TERM','EXAM', \
    'QCORRECT','TREATMENT','SoloTotal43.M1', \
    'Version','Gender','EYAinclude','d.version','NCRT']

# now make a dataframe out of it
df2 = pd.DataFrame(array2,columns=colnames)

df2.to_csv('C:\Users\Joss\ownCloud\Shared\DP_Derived_Data\dpLogisticDat_P101MT.csv',index=False,header=True)
