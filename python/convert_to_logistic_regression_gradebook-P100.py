...
df = pd.read_csv('C:\Users\Joss\ownCloud\Shared_Read_Only\DP_Anonymized_Data\P100 - 2017-09\dpDat.csv')
...
for index, row in df.iterrows():
    ...
    for iq in qnums:
        question = 'A'+str(iq)+'.f0' 
        trtcol = 'A'+str(iq)+'.trt.f0'
        ...
        if row[question]>=1:
            ...
            list2.append([row['AnonID'], \
                      ...
                      qcorrect, treatment, \
                      row['SoloTotal74.f0'], row['ExamGrade100.f0'], \
                      row['version.f0'], row['Gender'], row['EYAinclude.f0']
                      ])               
        elif row[question]==0:
            qcorrect = 0
            list2.append([row['AnonID'], \
...
colnames = ['ID','QNUM.0','QNUM.long','QNUM','COURSE','TERM','EXAM', \
    'QCORRECT','TREATMENT','SoloTotal74.f0','ExamGrade100.f0', \
    'version','Gender','EYAinclude']

# now make a dataframe out of it
df2 = pd.DataFrame(array2,columns=colnames)

df2.to_csv('C:\Users\Joss\ownCloud\Shared\DP_Derived_Data\dpLogisticDat_P100.csv',index=False,header=True)
